    ...
    def callback(callback):
        if callback.data not in mas_commands:
            ...
            if callback.data == "Add_yslyga_new_name":
                ...
                print("в добавлении"+str(add_yslyga_obsh))
...
#                 bot.send_message(callback.message.chat.id,test, parse_mode="HTML")
            ...
            #выводит свободное время для записи
                ...
                print(f"{master_r} {date_t} {yslyga}")
    ...
    def add_yslyga2(message):
        if message.text not in mas_commands:
            ...
            print(name_yslyga, opisanie_yslyga, cena_yslyga)
    ...
    #удаление услуги (получение ее названия и удаления из бд)
    name_del_yslyga = None
    # def delete_yslyga(message):
    #     global name_del_yslyga
    #     name_del_yslyga = message.text
    ...
    #     bot.send_message(message.chat.id, f'Успешно удалена услуга: <b>{name_del_yslyga}</b>.', parse_mode="HTML")
    ...
    def EndOfZapic3(message):
        if message.text not in mas_commands:
            ...
            print(f"{message_id} {master_r} {date_t} {time_t} {id_klient[0][0]}")
            cur.executescript(f'''INSERT INTO Записи(клиент_id,услуга_id,мастер_id,дата,время)
    select Клиенты.id, Свободные_записи.услуга_id, Свободные_записи.мастер_id, Свободные_записи.дата, Свободные_записи.время
    from Свободные_записи
    INNER JOIN Мастера ON Мастера.id = Свободные_записи.мастер_id
        ...
        if checkTime == True:
            ...
            for i in range(len(klient_id)):
                ...
                for j in range(len(napominanie)):
                    bot.send_message(klient_id[i][0], f"Фио: {napominanie[j][0]}\nДата: {napominanie[j][1]}\nВремя: {napominanie[j][2]}\nУслуга: {napominanie[j][3]}\nМастер:{napominanie[j][4]}",parse_mode="HTML")
